...
const loadCoupon = async () => {
  ...
  form.discount = response.discount;
  form.minimum_amount = response.minimum_amount;
  form.expires_at = response.expires_at_formated;
  form.active = response.active;
};

const submit = async () => {
  await couponStore.update(route.params.id, form);
};
  ...
  <main class="space-y-6">
    <div>
      ...
      <p class="mt-1 text-sm text-gray-500">
        Update a discount coupon for your customers.
      </p>
    </div>
    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div class="flex items-center justify-end gap-3">
        ...
        >
          ...
          {{ couponStore.loading ? "Updating..." : "Update Coupon" }}
        </button>
      </div>
    </form>
  </main>
